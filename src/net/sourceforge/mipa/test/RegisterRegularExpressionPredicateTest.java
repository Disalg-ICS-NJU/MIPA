...
public class RegisterRegularExpressionPredicateTest {
	 ...
	 * @throws Exception
	 */
	public void testRegisterRegularExpressionPredicate() throws Exception {
		server = MIPAResource.getNamingServer();
		MIPAResource.setCheckMode("lattice");
        ...
        Document predicate_2 = parseXml("config/predicate/predicate_sursequence.xml");
    	OnFire callback_2 = new OnFire();
        predicateID = broker.registerPredicate(callback_2, predicate_2);
        assertTrue(predicateID != null);
        assertTrue(groupManager.getPredicateInfo(predicateID)!=null);
        Thread.sleep(5000);
        assertTrue(callback_2.isFlag());
	...
	class OnFire extends UnicastRemoteObject implements ResultCallback {
		...
		private static final long serialVersionUID = 2375836470528021596L;
		boolean flag = false;
