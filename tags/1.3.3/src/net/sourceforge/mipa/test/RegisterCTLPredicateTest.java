...
public class RegisterCTLPredicateTest {
	...
	@Test
	/**
	 * 支持以时序逻辑（CTL）描述的环境动态变化特性
	 * @throws Exception
	 */
	public void testRegisterCTLPredicate() throws Exception {
        ...
        Document predicate_3 = parseXml("config/predicate/predicate_ctl_eu.xml");
    	OnFire callback_3 = new OnFire();
        predicateID = broker.registerPredicate(callback_3, predicate_3);
        assertTrue(predicateID != null);
        assertTrue(groupManager.getPredicateInfo(predicateID)!=null);
        Thread.sleep(5000);
        assertTrue(callback_3.isFlag());
	...
	class OnFire extends UnicastRemoteObject implements ResultCallback {
		...
		private static final long serialVersionUID = 5204397024579905427L;
		boolean flag = false;
